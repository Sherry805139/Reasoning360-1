#!/bin/bash
#SBATCH --job-name=example-multinode-rl-qwen7b-instruct-reasoning-gym-test
...
export PATH=/mnt/weka/home/jianshu.she/miniconda3/envs/Reasoning360/bin:$PATH
# =================== Cluster Environment ===================
...
# 使用ifbench数据
REASONING_GYM_TRAIN_PATH=/mnt/sharefs/users/jianshu.she/train_reasoning_gym.parquet
REASONING_GYM_TEST_PATH=/mnt/sharefs/users/jianshu.she/test_reasoning_gym.parquet

# All training files across all domains
train_files="['${REASONING_GYM_TRAIN_PATH}']"

# All test files across all domains
test_files="['${REASONING_GYM_TEST_PATH}']"
...
source /mnt/weka/home/jianshu.she/miniconda3/bin/activate Reasoning360

# =================== Logging ===================
WANDB_PROJECT=reasoning-gym-test
...
srun --nodes=1 --ntasks=1 -w "$head_node" --export=ALL --cpu-bind=none \
    bash -c "
        source /mnt/weka/home/jianshu.she/miniconda3/bin/activate Reasoning360
        if ! pip show reasoning_gym > /dev/null 2>&1; then
            echo 'reasoning_gym not found, installing...'
            pip install reasoning_gym
        fi
        echo '==== Node: $(hostname) ===='
        which python
        python --version
        pip show reasoning_gym || echo 'reasoning_gym NOT FOUND'
        python -c 'import reasoning_gym; print(\"reasoning_gym imported OK\")' || echo 'reasoning_gym import FAILED'
        ${CONDA_BIN_PATH}ray start --head --node-ip-address=\"$head_node_ip\" --port=$port --num-cpus \"${SLURM_CPUS_PER_TASK}\" --num-gpus 8 --include-dashboard=True --block
    " &
...
for ((i = 1; i < worker_num; i++)); do
    ...
    srun --nodes=1 --ntasks=1 -w "$node_i" --export=ALL --cpu-bind=none \
        bash -c "
            ...
            pip show reasoning_gym || echo 'reasoning_gym NOT FOUND'
            python -c 'import reasoning_gym; print(\"reasoning_gym imported OK\")' || echo 'reasoning_gym import FAILED'
            ${CONDA_BIN_PATH}ray start --address \"$address_head\" --num-cpus \"${SLURM_CPUS_PER_TASK}\" --num-gpus 8 --block
        " &
done
sleep 10
